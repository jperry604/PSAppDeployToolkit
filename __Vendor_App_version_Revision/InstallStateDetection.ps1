...
if(Test-Path $Filepathx86 -PathType Leaf) {
    if(([System.Version]::Parse((Get-Item $FilePathx86 -ea SilentlyContinue).VersionInfo.ProductVersionRaw)) -ge ([System.Version]::Parse($targetVersion)) ) {Write-Host "Installed"}
}
if(Test-Path $Filepath -PathType Leaf) {
    if(([System.Version]::Parse((Get-Item $FilePath -ea SilentlyContinue).VersionInfo.ProductVersionRaw)) -ge ([System.Version]::Parse($targetVersion)) ) {Write-Host "Installed"}
}
#>
